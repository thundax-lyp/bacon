...
class PaymentRemoteExceptionTranslatorTest {
    ...
    void shouldTranslateHttpStatusToDomainError() {
        RuntimeException translated = PaymentRemoteExceptionTranslator.translate(
                "createPayment",
    ...
    void shouldTranslateCircuitOpenToDomainError() {
        ...
        RuntimeException translated = PaymentRemoteExceptionTranslator.translate(
                "createPayment", CallNotPermittedException.createCallNotPermittedException(circuitBreaker));

        assertEquals(
                PaymentErrorCode.PAYMENT_REMOTE_CIRCUIT_OPEN.code(), ((PaymentDomainException) translated).getCode());
    }

    @Test
    void shouldTranslateUnknownToUnavailable() {
        RuntimeException translated =
                PaymentRemoteExceptionTranslator.translate("createPayment", new RuntimeException("boom"));

        assertEquals(
                PaymentErrorCode.PAYMENT_REMOTE_UNAVAILABLE.code(), ((PaymentDomainException) translated).getCode());
    }
}
