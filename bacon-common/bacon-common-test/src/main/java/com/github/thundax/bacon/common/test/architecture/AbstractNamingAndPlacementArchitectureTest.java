...
public abstract class AbstractNamingAndPlacementArchitectureTest {
    ...
    // `RepositoryImpl`：仓储实现，命名 `{业务对象}RepositoryImpl`，目录 `infra/repository/impl/`。
    @Test
    ...
    // `Mapper`：持久化映射，命名 `{业务对象}Mapper`，目录 `infra/repository/mapper/`。
    @Test
    ...
    // `Converter`：对象转换，命名 `{业务对象}Converter`，目录 `infra/repository/converter/`。
    @Test
    ...
    // `Facade`：跨域调用契约，命名 `{业务对象}{动作}Facade`，目录 `api/facade/`。
    @Test
    ...
    // `FacadeLocalImpl`：单体模式门面实现，命名 `{业务对象}{动作}FacadeLocalImpl`，目录 `interfaces/facade/`。
    @Test
    ...
    // `FacadeRemoteImpl`：微服务模式门面实现，命名 `{业务对象}{动作}FacadeRemoteImpl`，目录 `infra/facade/remote/`。
    @Test
