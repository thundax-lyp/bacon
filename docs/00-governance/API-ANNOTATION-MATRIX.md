...
| Provider | `@RestController @RequestMapping @Validated @Tag`；方法级 `@Operation` + HTTP Mapping；`interfaces.request.*Request` 参数声明 `@Valid`；完整路径遵守 `NAMING-AND-PLACEMENT-RULES.md` 的 provider path 规则 | `@HasPermission` |
