...
- 当一段代码同时包含“集合转换”和“对象组装”两层语义时，必须拆出具名中间变量，不得把完整 `stream` 转换直接内联到 `builder` 调用中。
- 这类中间变量的主要目的，是让 AI 和工程师都能直接看出每一步的语义边界，而不是减少代码行数。
- 只有在语义容易混淆、后续可能增加排序、校验、日志或调试断点的地方才增加这类变量；普通的一层链式调用不必机械拆开。
