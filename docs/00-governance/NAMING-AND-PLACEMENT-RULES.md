...
- `RepositoryImpl`：仓储实现，负责落库、查询、组装，命名 `{业务对象}RepositoryImpl`，默认目录 `infra/repository/impl/`；如与 `Mapper` / `DO` 强绑定，也允许放在 `infra/persistence/repository/impl/`，但同一业务域内目录语义必须保持一致
