...
- `recordCredentialFailure(tenantId, credentialId, reason)`
- `clearCredentialFailure(tenantId, credentialId)`
...
- `Controller`、`interfaces.facade`、`interfaces.provider` 固定负责把字符串 `tenantId` 解析为 `TenantId` 并校验租户存在性后再调用 `application`
...
- `application` 固定只接收 `TenantId`、`UserId`、`RoleId`、`DepartmentId`、`PostId` 等领域类型，不重复定义字符串协议参数重载
- 租户存在性校验固定在 `interfaces` 入口层完成，不在 `application` 服务内分散定义 `requireExistingTenantId`
...
- `User` 至少包含 `id`、`tenantId`、`account`、`name`、`phone`、`departmentId`、`avatarObjectId`、`status`、`deleted`
- `UserIdentity` 至少包含 `id`、`tenantId`、`userId`、`identityType`、`identityValue`、`status`
- `UserCredential` 至少包含 `id`、`tenantId`、`userId`、`identityId`、`credentialType`、`factorLevel`、`status`、`needChangePassword`、`failedCount`、`failedLimit`、`lockedUntil`、`expiresAt`
- `Tenant` 至少包含 `id`、`name`、`tenantCode`、`status`、`expiredAt`
- `Department` 至少包含 `id`、`tenantId`、`code`、`name`、`parentId`、`leaderUserId`、`sort`、`status`
...
- `UserIdentity.id` 固定为强类型 `UserIdentityId`，底层值使用字符串
- `UserIdentity.identityType` 固定使用领域枚举 `UserIdentityType`
- `UserIdentity.status` 固定使用领域枚举 `UserIdentityStatus`
- `UserIdentity(tenantId, identityType, identityValue)` 联合唯一
- `UserCredential.id` 全局唯一
- `UserCredential.id` 固定为强类型 `UserCredentialId`，底层值使用字符串
- `UserCredential.identityId` 固定关联强类型 `UserIdentityId`
- `UserCredential.credentialType` 固定使用领域枚举 `UserCredentialType`
- `UserCredential.factorLevel` 固定使用领域枚举 `UserCredentialFactorLevel`
- `UserCredential(identityId, credentialType)` 联合唯一
- `Tenant.id` 全局唯一
- `Tenant.id` 固定为强类型 `TenantId`，全局唯一
- `Tenant.tenantCode` 全局唯一
- `Tenant.tenantCode` 必须匹配 `[A-Z0-9_]+`
- `Department.id` 全局唯一
- `Department(tenantId, code)` 联合唯一
- `Post.id` 全局唯一
- `Post.id` 固定为强类型 `PostId`，底层值使用字符串
- `Post.status` 固定使用领域枚举 `PostStatus`
- `Post.code` 全局唯一
- `Role.id` 全局唯一
- `Role.id` 固定为强类型 `RoleId`，全局唯一
- `Role.roleType` 固定使用领域枚举 `RoleType`
- `Role.dataScopeType` 固定使用领域枚举 `RoleDataScopeType`
- `Role.status` 固定使用领域枚举 `RoleStatus`
- `Role.code` 全局唯一
- `Menu.id` 全局唯一
- `Menu.id` 固定为强类型 `MenuId`，底层值使用字符串
- `Menu.permissionCode` 全局唯一
- `Resource.id` 全局唯一
- `Resource.id` 固定为强类型 `ResourceId`，底层值使用字符串
- `Resource.resourceType` 固定使用领域枚举 `ResourceType`
- `Resource.status` 固定使用领域枚举 `ResourceStatus`
- `Resource.code` 全局唯一
...
- `UserCredential` 必须建立 `(tenantId, userId, status)`、`(tenantId, identityId, factorLevel, status)` 索引
...
- `UserIdentity` 的状态字段统一使用 `status`
- `UserCredential` 的状态字段统一使用 `status`
- `status` 固定值默认为 `ENABLED`、`DISABLED`，租户例外
- `UserIdentity.status` 固定值为 `ACTIVE`、`DISABLED`
- `UserCredential.status` 固定值为 `ACTIVE`、`LOCKED`、`EXPIRED`、`DISABLED`
- `UserCredential.status` 固定使用 `UserCredentialStatus`
- `status = DISABLED` 的对象不得继续产生新的业务生效结果
- `User.status = DISABLED` 时，该用户不得继续登录
- `Tenant.status = DISABLED` 时，该租户下用户不得继续登录
- `Tenant.status` 固定取值为 `ACTIVE`、`DISABLED`、`EXPIRED`
...
- 核心实体 `id` 全局唯一
- 主数据业务编码全局唯一
- `tenantId` 用于数据隔离，并与 `TenantId` 语义保持一致
...
- `UserIdentity` 必须包含 `identityType`、`identityValue`、`userId`、`tenantId`、`status`
- `UserIdentity.status = DISABLED` 时，该登录方式不可用于认证
...
- 单个缓存键只允许覆盖单个 `(tenantId, userId)` 读取上下文，不允许构建“租户全量权限大缓存”或“全局全量菜单大缓存”
