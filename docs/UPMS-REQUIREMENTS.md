...
- `getDepartmentById(tenantId, departmentId)`，返回固定 `DTO`
- `getDepartmentByCode(tenantId, departmentCode)`，返回固定 `DTO`
- `listDepartmentsByIds(tenantId, departmentIds)`，返回 `List<DTO>`

`DepartmentReadFacade` 返回值至少包含：
...
- `getRoleById(tenantId, roleId)`，返回固定 `DTO`
- `getRolesByUserId(tenantId, userId)`，返回 `List<DTO>`

`RoleReadFacade` 返回值至少包含：
...
- `permissionTargetType` 固定为 `menu`、`button`、`api/resource`
