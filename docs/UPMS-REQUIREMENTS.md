...
- `getUserMenuTree(tenantId, userId)`，返回树形 `DTO`
- `getUserPermissionCodes(tenantId, userId)`，返回 `Set<String>`
- `getUserDataScope(tenantId, userId)`，返回固定 `DTO`

`getUserDataScope` 返回值至少包含：
...
- `User.account` 全局唯一
- `UserIdentity` 按 `identityType + identityValue` 全局唯一
- `UserIdentity` 至少支持 `ACCOUNT`、`PHONE`、`WECOM`、`GITHUB`
...
### 6.5 Role, Menu, Resource, Data Scope

- 用户与岗位关系是一对多
- 角色类型固定为 `SYSTEM_ROLE`、`TENANT_ROLE`、`CUSTOM_ROLE`
- 内置 `Role` 不可删除
- 数据权限按 `Role` 配置，不直接配置给 `User`
- 数据权限先按 `tenantId` 收口，再计算范围
- 数据范围类型固定为 `ALL`、`DEPARTMENT`、`DEPARTMENT_AND_CHILDREN`、`SELF`、`CUSTOM`
- `CUSTOM` 当前只支持 `DEPARTMENT`
- `RoleDataScopeRelation` 只用于 `CUSTOM`
- 菜单类型固定为 `directory`、`menu`、`button`
- `directory/menu` 支持 `visible`
- `button` 不进入可见菜单树
- 权限目标类型固定为 `menu`、`button`、`api/resource`
- 资源类型固定为 `API`、`RPC`、`EVENT`
...
3. `Auth` 将身份信息写入 `token payload`，将权限数据写入缓存
4. 前端按菜单树渲染导航
5. 网关或业务服务按权限码鉴权
